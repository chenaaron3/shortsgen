...
export const runs = pgTable(t("runs"), {
  ...
  userId: text("userId")
    .notNull()
    .references(() => user.id, { onDelete: "cascade" }),
  /** Source label: full pasted text for text runs; site/page title line for URL runs (body comes from scrape in the pipeline). */
  user_input: text("user_input").notNull(),
  /** When ingest started from a URL: the https URL; pipeline fetches body without overwriting user_input. */
  source_url: text("source_url"),
  /** Ingest adapter used for URL runs: youtube, x, reddit, html, etc. */
  source_adapter: text("source_adapter"),
  /** Short display title for the run. */
  title: text("title"),
  /** Frozen pipeline config selected at create time (prototype/default). */
  config: text("config").default("prototype"),
  /** Frozen nugget cap computed at create time from balance. */
  max_nuggets: integer("max_nuggets").default(5),
  status: text("status").$type<RunStatus>().default("breakdown"),
  /** JSON array of playful loading messages for breakdown phase. */
...
export const videos = pgTable(t("videos"), {
  ...
  status: text("status")
    .$type<
      "created" | "scripts" | "assets" | "exporting" | "exported" | "failed"
    >()
    .default("created"),
  script: text("script"),
